...
3. Web Push VAPID public key lives in `lib/firebase/public-config.ts` (`vapidKey`).
   Rotate it from Firebase → Cloud Messaging → Web Push certificates if needed.
4. **Google Sign-In (required for the APK):** Firebase Console → Project settings → Your Android app → **Add fingerprint**
